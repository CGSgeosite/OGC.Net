...
namespace GMap.NET.MapProviders
{
    ...
    public class GMapProviders
    {
        ...
        {
            if (DbHash.TryGetValue(dbId, out var ret))
            {
                return ret;
            }
    ...
    public abstract class GMapProvider
    {
        ...
        public GMapProvider[] _overlays; //新增属性成员

        public float? Alpha = null; //新增属性成员

        public static List<GMapProvider> OverlayTiles = new(); //新增属性成员，以便在各类底图之上共享瓦片叠加层
        ...
        static readonly List<GMapProvider> MapProviders = new();
        ...
        /// <summary>
        ///     Time to live of cache, in hours. Default: 24*365 (365 days).
        /// </summary>
        public static int TTLCache = 24 * 365;
        ...
        protected PureImage GetTileImageUsingHttp(string url) 
        {
            ...
            if (CheckTileImageHttpResponse(response))
            {
                ...
                if (data.Length > 0)
                {
                    ret = TileImageProxy.FromStream(data, Alpha); //增补 透明度 alpha
