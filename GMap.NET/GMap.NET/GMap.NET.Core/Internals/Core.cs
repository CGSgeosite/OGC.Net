    ...
    internal sealed class Core : IDisposable
    {
        ...
        public GMapProvider Provider
        {
            ...
            set
            {
                ...
                {
                    var diffProjection = _provider == null || _provider.Projection != value.Projection;
        ...
        {
            var mmaxZoom = GetMaxZoomToFitRect(rect);
            if (mmaxZoom > 0)
            {
        ...
        public BackgroundWorker OnMapOpen()
        {
            if (!IsStarted)
            {
                var x = Interlocked.Increment(ref Instances);
        ...
        void InvalidatorWatch(object sender, DoWorkEventArgs e)
        {
            ...
            var spanMs = (int)span.TotalMilliseconds;
            var skiped = false;
            TimeSpan delta;
            DateTime now;
                    ...
                    if (_tileLoadQueue4Tasks == null)
                    {
                        ...
                        while (_tileLoadQueue4Tasks.Count < GThreadPoolSize)
                        {
                            ...
                            _tileLoadQueue4Tasks.Add(Task.Factory.StartNew(delegate()
                                {
                                    var ctid = "ProcessLoadTask[" + Thread.CurrentThread.ManagedThreadId + "]";
        ...
        static void ProcessLoadTask(LoadTask task, string ctid)
        {
            try
            {
                ...
                if (!m.NotEmpty)
                {
                    ...
                    var tileOverlays = task.Core._provider.Overlays;
                    ...
                    if (tileOverlays != null)
                        foreach (var tl in tileOverlays)
                    {
                        var retry = 0;
                        do
                        {
                            ...
                            {
                                var zoomOffset = task.Zoom > task.Core._okZoom ? task.Zoom - task.Core._okZoom : 1;
                                long ix = 0;
                                ...
                                {
                                    // offsets in quadrant
                                    var xOff = Math.Abs(task.Pos.X - parentTile.X * ix);
                                    var yOff = Math.Abs(task.Pos.Y - parentTile.Y * ix);
        ...
        void OnLoadComplete(string ctid)
        {
            ...
            var lastTileLoadTimeMs = (long)(_lastTileLoadEnd - _lastTileLoadStart).TotalMilliseconds;
        ...
        {
            var rez = Provider.Projection.GetGroundResolution(Zoom, Position.Lat);
        ...
        void Dispose(bool disposing)
        {
            if (IsStarted)
            {
                ...
                var x = Interlocked.Decrement(ref Instances);
